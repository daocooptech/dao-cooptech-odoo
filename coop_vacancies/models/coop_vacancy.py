...
class CoopVacancy(models.Model):
    ...
    def action_publish(self):
        ...
        """
        for record in self:
            publisher = record.partner_id
            if not publisher.coop_verified and not publisher.is_company:
                raise UserError(_(
                    'Вакансию размещает участник с подтверждённой личностью. '
                    'У «%(name)s» подтверждения нет — оно делается в профиле.',

...
class CoopTokenTransaction(models.Model):
    ...
    def write(self, vals):
        """Правится только пояснение.

        Сумма, основание, участник и ссылка на объект — это история, а не
        текущее состояние. Исправлять её задним числом значит потерять
        возможность объяснить остаток: по нему нельзя восстановить, откуда
        он взялся, а по истории остаток восстанавливается всегда.

        Ошибочное движение компенсируется встречным, с пояснением, — так
        же, как исправляют проводку в бухгалтерии.
        """
        protected = set(vals) - {'description'}
        if protected:
            raise UserError(_(
                'Движение токенов не изменяется: это история. Ошибку '
                'исправляют встречным движением с пояснением, а не правкой '
                'прежнего.'))
        return super().write(vals)

    def unlink(self):
        raise UserError(_(
            'Движение токенов не удаляется: по остатку без истории нельзя '
            'объяснить, откуда он взялся. Ошибку исправляют встречным '
            'движением.'))

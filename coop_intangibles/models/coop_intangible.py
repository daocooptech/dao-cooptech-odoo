...
class CoopIntangible(models.Model):
    ...
    **Оценка и обеспечение — разные вещи, и путать их дорого.** Оценка
    ...
    # Город берётся у правообладателя: своего у записи нет, а в каталоге он
    # нужен так же, как в остальных — по нему ищут и по нему
    # понимают, с кем придётся иметь дело.
    city = fields.Char(string='Город', related='owner_id.city', readonly=True)
    ...
    def _compute_is_mine(self):
        ...
        for record in self:
            record.is_mine = record.owner_id in mine
    ...
    def _search_is_mine(self, operator, value):
        ...
        if operator in ('=', 'in'):
            ...
            raise ValueError('Поддерживается только «моё: да» или «моё: нет».')
        return [('owner_id', 'in' if positive else 'not in',
    ...
    def _compute_can_request_license(self):
        ...
        for record in self:
            record.can_request_license = bool(
                record.state == 'active'
                and record.owner_id not in mine)

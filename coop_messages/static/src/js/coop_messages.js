...
export class CoopMessages extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            newDialogQuery: "",
            newDialogResults: [],
            // Переписки ещё едут. Пока признак поднят, на месте списка
            // крутится заставка, а не «Ничего не найдено»: пустой список
            // и незагруженный список выглядят одинаково, но говорят
            // человеку противоположное.
            threadsLoading: true,
        });
        ...
        onWillStart(async () => {
            ...
            // Дальше ждать нечего: запрос переписок уехал под отрисовку,
            // см. `onMounted` ниже.
        });

        // Переписки — после появления экрана, а не до.
        //
        // Владелец 22 сентября 2026: «модуль сообщений грузится долго».
        // Померено: запрос переписок стоит около полусекунды на сервере
        // (`/mail/data`, 56 запросов к базе), и раздел ждал его в
        // `onWillStart`, то есть не рисовался вовсе, пока он не придёт.
        // Полсекунды пустоты сверх пяти секунд загрузки платформы.
        //
        // Теперь экран появляется сразу — со списком разделов, поиском и
        // вкладками, — а переписки доезжают следом и список наполняется.
        // Панель разговора это переживает: она закрыта проверкой
        // `t-if="activeThread"`, и пока открытой переписки нет, там
        // пусто — ровно как при первом заходе человека, у которого
        // переписок ещё не было.
        //
        // Тот же приём, что уже применён к списку разделов бокового меню:
        // запрос, от которого зависит только наполнение, не должен
        // задерживать отрисовку. Ошибку здесь глотать нельзя молча —
        ...
        onMounted(() => {
            this.store.channels
                ...
                .finally(() => {
                    // Гасим и при отказе тоже: заставка, висящая вечно,
                    // хуже честного «ничего не найдено» — она обещает,
                    // что вот-вот появится.
                    this.state.threadsLoading = false;
                });
        });
    }

...
class CoopProjectContribution(models.Model):
    ...
    share_percent = fields.Float(
        string='Пай, %', compute='_compute_share_percent', store=True,
        digits=(5, 2),
        help='Вклад, делённый на сумму принятых вкладов проекта. Меняется, '
             'когда в проект вносят что-то ещё, — так и должно быть.')
    # То же число под другой подписью — для коммерческих проектов и ДАО.
    # Владелец 15 сентября 2026: «пай только в кооперативных проектах,
    # доля в коммерческих, а в некоммерческих вообще нет долей и паёв».
    # Слово не украшение: пай — имущественное участие в кооперативе с
    # возвратом по уставу, доля — часть в капитале общества.
    share_ratio = fields.Float(
        string='Доля, %', related='share_percent', readonly=True,
        digits=(5, 2))

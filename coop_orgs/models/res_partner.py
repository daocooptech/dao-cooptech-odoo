...
class ResPartner(models.Model):
    ...
    # Логотип бывает двух видов, и показываются они по-разному. Своя
    # картинка вписывается в плитку с полями на белом — иначе логотип
    # обрежется по краям. Монограмма (буква на фирменной подложке) должна
    # заливать плитку целиком: с полями она читалась бы как значок,
    # налепленный на пустую карточку.
    coop_has_own_logo = fields.Boolean(
        string='Свой логотип', default=False,
        help='Снято у организаций, для которых логотип не задан и вместо '
             'него показывается буква названия.')

...
# именем, прицел xpath мимо цели, объект заплатки, приложенный дважды или
# скопированный. Всё это ломает портал в браузере и до браузера
# выглядит исправным — 22 сентября 2026 три такие ошибки подряд уронили
# каталоги.
#
...
# Подстановки в строках. Та же беда другого рода: `_('%(орг)s', org=...)`
# компилируется, проходит любую проверку синтаксиса и падает `KeyError`
# ровно тогда, когда человек нажал кнопку. Четырнадцать таких мест
# оставило после себя переименование имён латиницей 22 сентября 2026.
if [ -f "$ODOO_HOME/coop-addons/tools/check_formats.py" ]; then
    if ! "$ODOO_HOME/venv/bin/python"             "$ODOO_HOME/coop-addons/tools/check_formats.py"; then
        say "ПОДСТАНОВКИ В СТРОКАХ НЕ СОШЛИСЬ — выкатка отменена"
        exit 1
    fi
fi

# Виды против схемы движка. Третья беда того же рода: вид, не прошедший
# схему, не встаёт на сервере с единственным словом в журнале — «Invalid
# view definition». 23 сентября 2026 на этом встали документы из-за
...
# выкатке, и цена этого видна в логе: сборка web.assets_web.min.js
# занимает 12 секунд, css — шесть, и платил их тот, кто зашёл первым.
# Удаление убрали, рассудив, что адрес пакета Odoo считает от содержимого
# файлов и при изменённом scss он и так становится другим.
#
# Для правки существующего файла это верно, для изменения состава — нет.
# 23 сентября 2026 замерено на боевой: выкатка добавила
# coop_theme/static/src/js/control_panel.js, откат его убрал — и после
# отката сервер продолжал отдавать пакет, в котором этот файл лежал.
# Клиент получал смесь старого кода с новым: оболочка не собиралась
# вовсе, ни одной ошибки ни в журнале сервера, ни в консоли браузера.
# Откат правки состояние не лечил — потому что откат и не пересобирал
# пакет.
#
# Поэтому сносим ровно тогда, когда состав файлов изменился: файл
# добавлен, удалён или переименован внутри static/, либо тронут
# манифест, где этот состав перечислен. Правка существующего файла
# по-прежнему ничего не сносит, и обычная выкатка стилей остаётся
# дешёвой. Десять секунд сборки при этом платит выкатка — прогрев
# ниже, — а не участник.
#
# Принудительно: COOP_FORCE_ASSETS=1 bash update.sh.
assets_changed=$(run git diff --name-status "$before" "$after"                  | grep -E '^[ADR]'                  | grep -E '/static/|__manifest__\.py'                  | head -1 || true)
if [ -n "$assets_changed" ] || [ -n "${COOP_FORCE_ASSETS:-}" ]; then
    say "Состав файлов пакета изменился — сношу собранные пакеты"
    sudo -u postgres psql -d "$DB" -qtAc         "delete from ir_attachment where url like '/web/assets/%';"         || say "Не удалось снести пакеты — прогрев ниже покажет, что отдаётся"
fi

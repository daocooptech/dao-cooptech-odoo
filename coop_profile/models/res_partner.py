...
class ResPartner(models.Model):
    ...
    # Переключатели приватности переехали в `coop_base`: их слушается не
    # только страница, но и окно «Показать контакты», а основа про
    # модуль страницы не знает. Решение 334 уточнено 20 сентября 2026.
    ...
    #
    # Имя в заголовке своей страницы — через связанное поле, а не `name`.
    #
    # Модуль автозаполнения компаний (`partner_autocomplete`, `_get_view`)
    # ставит свой виджет на любое поле `name` в любой форме контакта и тем
    # перебивает виджет из представления. Его виджет — однострочное поле
    # ввода, и на телефоне имя обрезалось: «Дашкевич Данил Игорев».
    # Связанное поле он не трогает, и заголовок правится по месту тем же
    # виджетом с переносом, что контакты ниже на странице.
    coop_name_edit = fields.Char(related='name', readonly=False,
                                 string='Имя и фамилия')

...
class ResPartner(models.Model):
    ...
    coop_is_participant = fields.Boolean(
        ...
        help='Человек зарегистрирован на платформе и виден в каталоге людей. '
             'Контакт без этого признака остаётся обычным контактом и в '
             'каталог не попадает.')

    coop_birthdate = fields.Date(string='Дата рождения')
    coop_age = fields.Integer(string='Возраст', compute='_compute_age')

    # Профессия — кем человек работает; навыки — что он берётся делать.
    # В каталоге это разные оси: по профессии выбирают, к кому идти, по
    # навыкам — кто именно из них справится с конкретной задачей.
    coop_profession_id = fields.Many2one(
        'coop.profession', string='Профессия', index=True, ondelete='restrict')
    coop_profession_category_id = fields.Many2one(
        'coop.profession.category', string='Категория специализации',
        related='coop_profession_id.category_id', store=True, index=True)

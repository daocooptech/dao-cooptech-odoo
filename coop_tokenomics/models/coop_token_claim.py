...
class CoopTokenClaim(models.Model):
    ...
    **Токен — Jetton (TEP-74), делимый.** Один выпуск — один
    ...
    settlement_currency = fields.Selection([
        ...
        ('usdt', 'USDT'),
        ('ton', 'TON'),
    ], string='Валюта расчёта', default='rub', required=True,
        help='Расчёт в рублях: участник считает поставку в той же валюте, '
             'в какой ведёт хозяйство, и цена обещания сопоставима с ценой '
             'того же товара в каталоге. Рублёвого токена в сети TON нет, '
             'поэтому рублёвый расчёт идёт зачётом внутри платформы либо '
             'через оператора; в самой сети остаётся расчёт в USDT или TON '
             'для тех, кто торгует между узлами.')

...
class CoopMembership(models.Model):
    ...
    def action_coop_open_organization(self):
        """Со страницы человека — на страницу организации, а не в окно.

        В полке «Организации» карточки рисуются по записям членства:
        снимок, название и роль берутся у организации, но запись — это
        членство. Встроенный канбан по щелчку открывает во всплывающем
        окне саму запись, то есть членство; человек же ждёт страницу
        организации — так в макете, и так работает любая другая полка.

        Вид берём у раздела организаций, а не оставляем на усмотрение
        движка: карточка организации — это её страница, а не форма
        контакта из «Контактов». Ссылка мягкая: `coop_base` не зависит
        от `coop_orgs`, и без него страница просто откроется видом по
        умолчанию, а не свалится на «Внешний идентификатор не найден».
        """
        self.ensure_one()
        организация = self.organization_id
        if not организация:
            raise UserError(_('У этого членства не указана организация.'))
        вид = self.env.ref('coop_orgs.view_coop_orgs_form',
                           raise_if_not_found=False)
        return {
            'type': 'ir.actions.act_window',
            'name': организация.display_name,
            'res_model': 'res.partner',
            'res_id': организация.id,
            'view_mode': 'form',

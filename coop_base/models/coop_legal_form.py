...
class CoopLegalFormGroup(models.Model):
    ...
    # Как вид называется на карточке и как называется её состав.
    #
    # В макете под названием организации стоит «Кооперативная
    # организация», а полка состава подписана «Пайщики» — у коммерческой
    # «Сотрудники», у некоммерческой «Участники». Слово не украшение:
    # пайщик вносит пай и голосует, наёмный сотрудник — ни того ни
    # другого, и называть их одинаково значит обещать не то.
    #
    # Хранится у группы форм, а не выводится из кода в шаблоне: групп
    # четыре, и пятая появится вместе с новой правовой формой, а не
    # вместе с правкой разметки.
    card_label = fields.Char(
        string='Вид на карточке',
        help='Подпись под названием организации: «Кооперативная '
             'организация», «Коммерческая организация».')
    member_label = fields.Char(
        string='Как зовётся состав',
    ...
    CAPTIONS = {
        ...
        'decentralized': ('Децентрализованная организация', 'Участники'),
    }

    def init(self):
        """Проставить подписи тем группам, у которых их нет.

        Справочник правовых форм загружен с `noupdate="1"` — и правильно:
        владельцу позволено править формы, и обновление модуля не должно
        затирать его правки. Но из-за этого новые поля в уже загруженные
        записи не попадают вовсе, и подпись вида на карточке остаётся
        пустой.

        Поэтому заполняем здесь и только пустые: правку владельца это не

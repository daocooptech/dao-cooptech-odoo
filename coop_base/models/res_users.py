...
class ResUsers(models.Model):
    ...
    def _coop_partner_ids(self):
        """Чьё считать «моим».

        Свой профиль и организации, где у человека есть хоть одно
        полномочие, которым действуют от их имени. По этому списку
        отбирается всё, что показывается во вкладках «Мои склады», «Мои
        закупки», «Организую».

        Метод звали из складов, биржи мощностей и боковой полосы — и его
        не существовало. Падало это не везде: боковая полоса обходила
        `hasattr`, а склады молча роняли отбор. Нашлось 15 сентября 2026,
        когда по тому же образцу писалась вкладка «Организую».

        Возвращает список номеров, а не набор записей: все, кто его
        зовёт, кладут результат прямо в домен.
        """
        self.ensure_one()

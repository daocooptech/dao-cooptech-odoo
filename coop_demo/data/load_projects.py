...
def load_projects(env, extra=100):
    ...
    for index, row in enumerate(rows + _extra_rows(rows, extra, rnd)):
        ...
        if project:
            ...
            # Владельца и автора — только при заведении. Инициатор берётся
            # по номеру из списка участников, а список растёт от прогона к
            # прогону, — и каждое обновление переписывало владельца у
            # полутора сотен проектов, а выравнивание полок перекладывало
            # их обратно (замер 26.09, решение 416: «загрузчики доливают
            # данные»).
        ...
        # Состояние — только новому проекту. Существующим его уже довели
        # другие загрузчики (запуск, завершение, заморозка — `load_examples`),
        # и переписывать его здесь значило бы каждый прогон откатывать их
        # работу: восемь проектов менялись «завершён ↔ запущен» при каждом
        # обновлении (замер 26.09, решение 416).
        if not is_new:
            continue
        if state == 'frozen':

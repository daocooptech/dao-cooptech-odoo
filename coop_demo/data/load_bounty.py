...
# проставлением поля.
СОСТОЯНИЯ = (['published'] * 7) + ['draft', 'submitted', 'accepted']
ВОЗНАГРАЖДЕНИЯ = [40, 60, 80, 100, 120, 150, 180, 200, 250, 300]


def ещё_задачи(сколько=112):
    """Список кортежей той же формы, что и `TASKS`."""
    собранные = []
    for номер in range(сколько):
        название, специализация = ЕЩЁ[номер % len(ЕЩЁ)]
        if номер >= len(ЕЩЁ):
            название = '%s (%s)' % (название, ГОРОДА[номер % len(ГОРОДА)])
        собранные.append((
            название,
            ОПИСАНИЯ[номер % len(ОПИСАНИЯ)],
            специализация,
            ВОЗНАГРАЖДЕНИЯ[номер % len(ВОЗНАГРАЖДЕНИЯ)],
            СОСТОЯНИЯ[номер % len(СОСТОЯНИЯ)],
        ))
    return собранные

...
PEOPLE = [
    ...
    'Могу участвовать трудом в проекте, если нужен {spec} на несколько смен.',
...
# Прежние формулировки записей загрузчика и то, чем их заменить.
# Владелец 24 сентября 2026: «Готов вложиться» → «Могу вложиться» — «это
# надо переименовать в Участвовать».
RENAMES = [
    ('Готов вложиться трудом в проект', 'Могу участвовать трудом в проекте'),
    ('Могу вложиться трудом в проект', 'Могу участвовать трудом в проекте'),
]


def _rename_phrases(env):
    """Переписать уже заведённые записи загрузчика под новые формулировки.

    Только записи самого загрузчика (заведены от имени системы): записи
    людей не трогаются. Повторный запуск ничего не меняет.
    """
    Message = env['mail.message'].sudo()
    system = (env.ref('base.user_root') | env.ref('base.user_admin')).ids
    fixed = 0
    for old, new in RENAMES:
        for post in Message.search([
                ('model', '=', 'res.partner'), ('message_type', '=', 'comment'),
                ('create_uid', 'in', system), ('body', 'ilike', old)]):
            post.body = str(post.body).replace(old, new)
            fixed += 1
    if fixed:
        _logger.info('Стены: переписано записей %s', fixed)

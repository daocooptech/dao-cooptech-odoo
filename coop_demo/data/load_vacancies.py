...
# неправдой.
# Третий элемент — слово, по которому вакансия находит настоящий сбор
# вкладов. Точного совпадения названий ждать нельзя: в макете «3D», в
# каталоге «3д», и по равенству строк они не встретятся никогда.
DEMO_PROJECTS = [
    ('Строительный 3D-принтер', 12000000, 'принтер'),
    ('Круглогодичная теплица', 8500000, 'теплиц'),
    ('Кооперативный склад', 5000000, 'склад'),
]


def _ensure_projects(env):
    """Проекты, к которым привязаны вакансии.

    Сначала ищем настоящий сбор вкладов и берём выросший из него проект
    в управлении. Так вакансия попадает в живой проект со вкладчиками,
    сроками и лентой, а не в пустую карточку, заведённую ради поля.

    Заглушка создаётся только если подходящего сбора на узле нет вовсе:
    без проекта доля исполнителя не считается ни от чего, и показывать
    вместо процента ноль было бы неправдой.
    """
    Project = env['project.project'].sudo()
    Collect = env['coop.project'].sudo()
    projects = []
    for name, total, token in DEMO_PROJECTS:
        ...
        if collect:
            ...
            # Сумма вкладов берётся у сбора, а не из списка выше: там
            # круглое число на случай, когда сбора нет, а здесь —
            # настоящая, посчитанная по принятым вкладам.
            project.coop_contribution_total = collect.contribution_total

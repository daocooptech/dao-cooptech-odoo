...
class CoopGroupBuy(models.Model):
    ...
    # Рубрика складчины. Своя, а не общий справочник ресурсов: вскладчину
    # берут не то же, что продают поштучно, — корма, топливо, тару, — и в
    # разделах каталога ресурсов этих строк просто нет. Город рубрикой не
    # годится: по нему каталог рассыпается на четыре десятка полок по три
    # записи, а полка из трёх карточек выглядит ошибкой.
    category = fields.Selection([
        ('feed', 'Корма'),
        ('grocery', 'Продовольствие и бакалея'),
        ('fuel', 'Топливо'),
        ('seed', 'Семена и удобрения'),
        ('build', 'Стройматериалы'),
        ('pack', 'Тара и упаковка'),
        ('other', 'Прочее'),
    ], string='Раздел', default='other', required=True, index=True)

...
export class CoopCatalogKanbanController extends KanbanController {
    ...
    /** Полки — витрина над каталогом: показываются на чистом экране, а как
     *  только человек искал или отбирал, пропадают. Он уже знает, что ищет,
     *  и рубрики ему мешают. */
    get coopShelvesVisible() {
        if (!this.coopShelfField) {
            return false;
        }
        // Раскладка каталога здесь ни при чём, и это было причиной, по
        // которой полок не было видно неделю. В условии стояло
        // `mode === "tiles"`, а каталоги открыты в строчном виде: режим
        // общий на всю платформу и хранится в браузере, владелец выбрал
        // строки. Код полок работал, признак доходил, группы с сервера
        // приходили — не совпадало одно это условие.
        //
        // Витрина стоит над каталогом и от того, как показан сам каталог
        // ниже, не зависит: рубрики отвечают «что здесь бывает», а плитка
        // или строка — это про то, как читать конкретную запись.
        return !this.coopSearchActive;
    }

    /** Ищет ли человек прямо сейчас.
     *
     * Обёрнуто целиком: в прошлый раз обращение к поисковой модели из
     * геттера, вычисляемого при отрисовке, оставило экран без полок и без
     * объяснений. Если спросить не вышло — считаем, что поиск пуст:
     * лишняя витрина лучше пропавшей.
     */
    get coopSearchActive() {
        try {
            const model = this.env.searchModel;
            if (!model) {
                return false;
            }
            const facets = typeof model.facets === "object" ? model.facets : null;
            return Boolean(facets && facets.length);
        } catch {
            return false;
        }
    }

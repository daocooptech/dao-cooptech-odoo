...
export class CoopCatalogKanbanController extends KanbanController {
    ...
    /** Значок раздела для полок каталога без снимков. Объявляется в
     *  действии рядом с полем рубрики, потому что это то же самое
     *  решение: чем этот каталог показывает запись, у которой нет
     *  снимка. Карточка каталога берёт его оттуда же. */
    get coopShelfIcon() {
        return this.props.context?.coop_shelf_icon || false;
    }
    ...
    get coopSearchIsClean() {
        ...
        // нельзя.
        return !(this.env.searchModel?.facets || []).length;
    }

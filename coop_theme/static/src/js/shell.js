...
export class CoopTabs extends Component {
    ...
    setup() {
        ...
        // Действия, ради которых меню уже перечитывали. Множество, а не
        // флаг: экранов за сессию много, и одна неудача не должна
        // запрещать попытку на следующем.
        this.reloadedFor = new Set();
        this.refresh();
    ...
    refresh() {
        ...
        // Меню хранится у участника в браузере, и после переноса раздела
        // на движок оно там остаётся прежним: новых пунктов в нём нет, а
        // значит нет и вкладок. Так и вышло с «Токеномикой» — на сервере
        // четыре пункта, у участника ни одного, и строка вкладок пустая
        // без единой ошибки.
        //
        // Поэтому: не нашли действие среди пунктов — перечитываем меню и
        // пробуем ещё раз. Один раз на открытие экрана, иначе на
        // служебных действиях движка (у них своего пункта меню нет и не
        // будет) перезагрузка ходила бы по кругу.
        if (!own && actionId && !this.reloadedFor.has(actionId)) {
            this.reloadedFor.add(actionId);
            this.menus.reload().then(() => this.refresh());
            return;
        }

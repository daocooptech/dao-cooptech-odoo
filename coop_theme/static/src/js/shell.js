...
patch(WebClient.prototype, {
    setup() {
        ...
        onMounted(() => {
            ...
            // Состояние меняем не сразу, а следующим тактом.
            //
            // Наблюдатель просыпается посреди отрисовки: разметку меняет
            // сам движок, пока собирает экран. Запись в общее состояние в
            // этот момент отменяет идущую отрисовку и начинает её заново —
            // и полоса управления рисуется по контроллеру, у которого
            // сборка ещё не закончилась. В браузере это выглядит как
            // «Упс! Что-то пошло не так», а в консоли как
            // `Invalid loop expression: "undefined" is not iterable`:
            // перебор по полю, которое контроллер не успел себе завести.
            //
            // Ровно эта ловушка уже записана в `catalog_shelves.js` про
            // полки — потомок трогал состояние, от которого зависит
            // отрисовка родителя, пока она шла. Здесь то же самое, только
            // источник не компонент, а наблюдатель за разметкой.
            //
            // Такт ожидания ничего не стоит на глаз и разводит запись с
            // отрисовкой.
            // Состояние меняется не сразу, а следующим тактом.
            //
            // Наблюдатель просыпается посреди отрисовки: разметку меняет
            // сам движок, пока собирает экран. Запись в общее состояние в
            // этот момент отменяет идущую отрисовку и начинает её заново —
            // а полоса управления при этом рисуется по контроллеру,
            // который ещё не закончил сборку. В браузере это «Упс! Что-то
            // пошло не так», в консоли — `Invalid loop expression:
            // "undefined" is not iterable`: перебор по полю, которое
            // контроллер не успел себе завести.
            //
            // Ровно эта ловушка записана в `catalog_shelves.js` про полки:
            // потомок трогал состояние, от которого зависит отрисовка
            // родителя, пока она шла. Здесь то же самое, только источник
            // не компонент, а наблюдатель за разметкой.
            //
            // Такт ожидания незаметен на глаз и разводит запись с
            // отрисовкой. Заодно он схлопывает пачку срабатываний
            // наблюдателя в один пересчёт: движок правит разметку
            // десятками мелких шагов, и считать на каждом незачем.
            const применить = () => {
            ...
            const пересчитать = () => {
                browser.clearTimeout(this.coopLoaderApply);
                this.coopLoaderApply = browser.setTimeout(применить, 0);
            };

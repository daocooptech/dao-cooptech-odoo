...
const SORT_OPTIONS = {
    ...
    // У торгов «сначала новые» — худший из возможных порядков: из ста
    // пяти лотов идут семь, и на витрине стояли одни завершённые, а до
    // идущих человек не долистывал вовсе. Раздел, в котором нельзя
    // сделать ставку, — не торги, а архив. Поэтому первым стоит порядок
    // по состоянию: он же становится порядком по умолчанию.
    "coop.auction": [
        ["live_rank asc, date_end asc", "Сначала идущие"],
        ["date_end asc", "Скоро заканчиваются"],
        ["id desc", "Сначала новые"],
        ["current_price desc", "Сначала дорогие"],
        ["city asc", "По городу"],
    ],
    // У расширения нет ни города, ни даты события: выбирают по разделу,
    // по названию и по цене. Первым стоит раздел — он же порядок по
    // умолчанию в самом представлении, и список не должен перестраиваться
    // от одного открытия панели.
    "coop.extension": [
        ["category, sequence, name", "По разделу"],
        ["name asc", "По названию"],

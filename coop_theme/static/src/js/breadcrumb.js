...
patch(Breadcrumbs.prototype, {
     ...
     *
     * От этого зависит, что остаётся в полосе над карточкой. Решения
     * владельца 21 сентября 2026, два подряд:
     *
     * 1. Путь убрать. Пройдя «мои друзья → друзья друга», он увидел
     *    «Дашкевич → Соколов», стоя на странице Медведевой: движок
     *    пишет в путь предков, а имя открытой записи берёт из
     *    заголовка, которого у карточки человека нет — имя стоит в
     *    самой странице, крупно. Выходит путь, называющий кого угодно,
     *    кроме того, кто перед глазами. Вместе с путём уходит и
     *    последняя крошка: она и есть это пустое имя.
     * 2. Шестерёнку убрать. Это штатное «Меню действий» движка, и на
     *    карточке участника в нём «Отправить SMS» (платный сервис, не
     *    подключён), «Загрузить (vCard)» и «Предоставить доступ к
     *    порталу» — последнее чисто административное. Ни одно из трёх
     *    к странице участника отношения не имеет.
     *
